...
export function argdown(obj: string) {
    ...
    var nodes: { [key: string]: model.Node } = Object.fromEntries(response.map!.nodes.filter((value) => inEdges(value, response.map!.edges)).map(
        (n) => {
...
function inEdges(node: IMapNode, edges: Array<IMapEdge>): boolean {
    return edges.find((value) => value.from.id === node.id || value.to.id === node.id) !== undefined;
}
